import create_godot_minor_release from '../../src/archived/create-godot-minor-release';
...
describe('Create Godot Minor Release - Happy Paths w/ Mocks', () => {
  beforeEach(() => {
  ...
  test('Integration Test', async () => {
    const response = await create_godot_minor_release();
...
describe('Create Godot Minor Release - Failure Paths', () => {
  beforeEach(() => {
  ...
  test('File does not exist - macu-secrets.json', async () => {
    expect((await create_godot_minor_release()).body).toBe('[macu cgmr]: Could not find macu-secrets.json in your home directory. Please create your personal access token and try again');
  });
  ...
  test('File does not exist - project.godot', async () => {
    ...
    expect((await create_godot_minor_release()).body).toBe('[macu cgmr]: Required file project.godot does not exist in current working directly. Please try again in the current directoy');
  });
  ...
  test('Misaligned Versions', async () => {
    ...
    expect((await create_godot_minor_release()).body).toBe('[macu cgmr]: Github tag and Godot versions are misaligned. Please fix this misalignment and try again');
  });
});